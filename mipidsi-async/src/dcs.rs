...
pub trait InterfaceExt: Interface {
    ...
    fn write_command(&mut self, command: impl DcsCommand) -> impl core::future::Future<Output = Result<(), Self::Error>> {async move {
    ...
    /// command set.
    fn write_raw(&mut self, instruction: u8, param_bytes: &[u8]) -> impl core::future::Future<Output = Result<(), Self::Error>> {async move {
