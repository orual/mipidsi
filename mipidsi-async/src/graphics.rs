    ...
    RST: OutputPin,
{
        ...
        I: IntoIterator<Item = Self::Color>,
    {
        ...
        if &intersection == area {
            ...
            block_on(self.set_pixels(sx, sy, ex, ey, take_u32(colors, count)))
        } else {
    ...
    fn fill_solid(&mut self, area: &Rectangle, color: Self::Color) -> Result<(), Self::Error> {
        ...
        block_on(self.set_address_window(sx, sy, ex, ey))?;
        block_on( self.di.write_command(WriteMemoryStart))?;
        block_on(M::ColorFormat::send_repeated_pixel(&mut self.di, color, count))
    }
}
