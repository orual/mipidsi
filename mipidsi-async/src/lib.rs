...
//! ```
//! use mipidsi_async::interface::SpiInterface;                    // Provides the builder for DisplayInterface
//! use mipidsi_async::{Builder, models::ILI9486Rgb666};           // Provides the builder for Display
...
//!# let spi = mipidsi_async::_mock::MockSpi;
//!# let dc = mipidsi_async::_mock::MockOutputPin;
...
//! use mipidsi_async::interface::{Generic8BitBus, ParallelInterface};
//! // Provides the builder for Display
//! use mipidsi_async::{Builder, models::ILI9341Rgb666};
...
//! `rst` */
//!# tokio_test::block_on(async {
//!# let lcd_d0 = mipidsi_async::_mock::MockOutputPin;
//!# let lcd_d1 = mipidsi_async::_mock::MockOutputPin;
//!# let lcd_d2 = mipidsi_async::_mock::MockOutputPin;
//!# let lcd_d3 = mipidsi_async::_mock::MockOutputPin;
//!# let lcd_d4 = mipidsi_async::_mock::MockOutputPin;
//!# let lcd_d5 = mipidsi_async::_mock::MockOutputPin;
//!# let lcd_d6 = mipidsi_async::_mock::MockOutputPin;
//!# let lcd_d7 = mipidsi_async::_mock::MockOutputPin;
//!# let wr = mipidsi_async::_mock::MockOutputPin;
//!# let dc = mipidsi_async::_mock::MockOutputPin;
//!# let rst = mipidsi_async::_mock::MockOutputPin;
//!# let mut delay = mipidsi_async::_mock::MockDelay;
//!
...
//!      .reset_pin(rst)
//!      .color_order(mipidsi_async::options::ColorOrder::Bgr)
//!      .init(&mut delay).await.unwrap();
//!
//! // Clear the display to black
...
mod builder;
pub use builder::{Builder, NoResetPin, InitError};
    ...
    RST: OutputPin,
{
    ...
    /// ```
    ///# tokio_test::block_on(async {
    /// use mipidsi_async::options::{Orientation, Rotation};
    ///
    /// # let mut display = mipidsi_async::_mock::new_mock_display().await;
    ...
    ///
    /// # let mut display = mipidsi_async::_mock::new_mock_display().await;
    /// display.set_pixel(100, 200, Rgb565::new(251, 188, 20)).await.unwrap();
    /// # })
    /// ```
    ...
    async fn set_address_window(&mut self, sx: u16, sy: u16, ex: u16, ey: u16) -> Result<(), DI::Error> {
        ...
        self.di.write_command(dcs::SetColumnAddress::new(sx, ex)).await?;
        self.di.write_command(dcs::SetPageAddress::new(sy, ey)).await
    }
    ...
    pub async fn set_tearing_effect(
        ...
        self.di
            .write_command(dcs::SetTearingEffect::new(tearing_effect)).await
    }
